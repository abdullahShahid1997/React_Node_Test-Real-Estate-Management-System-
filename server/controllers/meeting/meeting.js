...
const add = async (req, res) => {
  try {
    ...
    const { agenda, attendes, attendesLead, location, related, dateTime, notes, createBy } =
      req.body;
    ...
    if (!agenda || !createBy || !dateTime) {
      return res
        .status(400)
        .json({ success: false, message: "Agenda, createBy, and dateTime are required fields" });
    }
    ...
    if (!Array.isArray(attendes) || !Array.isArray(attendesLead)) {
      return res
        .status(400)
        .json({ success: false, message: "Attendes and attendesLead should be arrays" });
    }
...
const deleteMany = async (req, res) => {
  try {
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid request, ids must be a non-empty array" });
    }
